...
impl UnlitShader {
    ...
    pub fn render_billboard(&self, ctx: &RenderContext, obj: &Billboard) {
        unsafe {
            ctx.use_program(Some(self.program));
    ...
    pub fn render_ui(&self, ctx: &RenderContext, transform: &Transform, primitive: &Primitive) {
        unsafe {
            ctx.use_program(Some(self.program));

            primitive.bind(ctx, &self.sprite_sheet_uniforms);
            ctx.ui_cam.bind(ctx, &self.camera_uniforms);
            transform.bind(ctx, &self.model_loc);
